...
const navigate = useNavigate();
  ...
  const handleSubmit = async (e) => {
     ...
     const response = await api.post("/register",
      ...
      if(error.response){
        console.error("registration failed:", error.responce.data.message || "email is already exits");
     ...
     <div className='min-h-screen  flex  justify-center items-center bg-gradient-to-r from-cyan-500 to-blue-600 '>
        <div className ="shadow-md w-full max-w-md rounded-xl bg-white p-7" >
          ...
          <form onSubmit ={handleSubmit} action ="/register" className='mt-6'>
            ...
            <input type="text" name="firstname" placeholder = "Enter Your Name" className='w-full px-4 py-2 border-1 rounded-lg focus:outline-none fous:ring-2 focus:ring-black-100'
              onChange={(e) => setFirstname(e.target.value)}/>
            <label className='text-black text-xl font-semibold block mb-2'>Username</label>
            <input type="text" name="username" placeholder = "Enter Your Name" className='w-full px-4 py-2 border-1 rounded-lg focus:outline-none fous:ring-2 focus:ring-black-100'
